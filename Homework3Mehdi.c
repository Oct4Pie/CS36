...
float calc_tax(float gross) {
    return gross * 0.2;
}

void get_input(int num_employees, char (*names)[15], float* hourly_rates, float* hours_worked, int* num_entered) {
...
int main() {
    ...
    get_input(num_employees, names, hourly_rates, hours_worked, &num_entered);

    for (int i = 0; i < num_employees; i++) {
        calc(hourly_rates[i], hours_worked[i], taxes + i, nets + i, gross + i, over_works + i);
        taxes[i] = calc_tax(gross[i]);
        nets[i] = gross[i] - taxes[i];
    }
...
Please enter employee 2's name: -1
